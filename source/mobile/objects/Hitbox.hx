...
class Hitbox extends MobileInputManager
{
	...
	public var extraKey1 = CDevConfig.saveData.extraKeyReturn1.toUpperCase();
	public var extraKey2 = CDevConfig.saveData.extraKeyReturn2.toUpperCase();
	public var extraKey3 = CDevConfig.saveData.extraKeyReturn3.toUpperCase();
	public var extraKey4 = CDevConfig.saveData.extraKeyReturn4.toUpperCase();
	...
	public function new(?CustomMode:String):Void
	{
		...
		if ((CDevConfig.saveData.hitboxmode != 'Classic' && CustomMode == null) || CustomMode != null){
			var Custom:String = CustomMode != null ? CustomMode : CDevConfig.saveData.hitboxmode;
			...
			if (CDevConfig.saveData.extraKeys == 1 && MobileData.hitboxModes.get(Custom).single != null)
				currentHint = MobileData.hitboxModes.get(Custom).single;
			if (CDevConfig.saveData.extraKeys == 2 && MobileData.hitboxModes.get(Custom).double != null)
				currentHint = MobileData.hitboxModes.get(Custom).double;
			if (CDevConfig.saveData.extraKeys == 3 && MobileData.hitboxModes.get(Custom).triple != null)
				currentHint = MobileData.hitboxModes.get(Custom).triple;
			if (CDevConfig.saveData.extraKeys == 4 && MobileData.hitboxModes.get(Custom).quad != null)
				currentHint = MobileData.hitboxModes.get(Custom).quad;
			if (CDevConfig.saveData.extraKeys != 0 && MobileData.hitboxModes.get(Custom).test != null)
			...
			for (buttonData in currentHint)
			{
				...
				var location = CDevConfig.saveData.hitboxLocation;
				...
				for (i in 1...9) {
					var button = Reflect.getProperty(CDevConfig.saveData, 'extraKeyReturn${i}');
					if (customReturn == null && buttonData.button == 'buttonExtra${i}') customReturn = button.toUpperCase();
				}

				if (CDevConfig.saveData.extraKeys == 0 && buttonData.extraKeyMode == 0 ||
				   CDevConfig.saveData.extraKeys == 1 && buttonData.extraKeyMode == 1 ||
				   CDevConfig.saveData.extraKeys == 2 && buttonData.extraKeyMode == 2 ||
				   CDevConfig.saveData.extraKeys == 3 && buttonData.extraKeyMode == 3 ||
				   CDevConfig.saveData.extraKeys == 4 && buttonData.extraKeyMode == 4)
				{
					addButton = true;
				}
				...
				for (i in 1...9) {
					...
					if (buttonData.button == buttonString && customReturn == null)
						customReturn = Reflect.getProperty(CDevConfig.saveData, 'extraKeyReturn${i}');
				}
	...
	{
		var guh:Float = CDevConfig.saveData.hitboxalpha;
		var shape:Shape = new Shape();
		shape.graphics.beginFill(Color);
		switch (CDevConfig.saveData.hitboxtype) {
			case "No Gradient":
	...
	private function createHint(X:Float, Y:Float, Width:Int, Height:Int, Color:Int = 0xFFFFFF, ?customReturn:String, ?mapKey:String):MobileButton
	{
		...
		hint.onDown.callback = function()
		{
			...
			if (hint.alpha != CDevConfig.saveData.hitboxalpha)
				hint.alpha = CDevConfig.saveData.hitboxalpha;
		}
