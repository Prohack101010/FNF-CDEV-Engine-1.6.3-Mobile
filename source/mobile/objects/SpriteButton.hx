 ...
 */
enum abstract SpriteButtonStatus(Int) from Int to Int
{
	var NORMAL = 0;
	var PRESSED = 1;
}
 ...
 * Alternive of MobileButton for making buttons with sprites more easily.
 */
#if !display
@:generic
#end
@:allow(mobile.objects.SpriteButton)
class SpriteButton extends FlxSprite implements IFlxInput
{
	/**
	 * A map that's storing every active touch's ID that's pressing a button.
	 */
	public static var buttonsTouchID:Map<Int, SpriteButton> = new Map();

	/**
	 * The current state of the button, either `SpriteButtonStatus.NORMAL` or `SpriteButtonStatus.PRESSED`.
	 */
	public var status:SpriteButtonStatus;
	...
	/**
	 * Creates a new `SpriteButton` object.
	 *
	...
	public function new(x:Float = 0, y:Float = 0):Void
	{
		...
		status = SpriteButtonStatus.NORMAL;
		solid = false;
		immovable = true;
		#if FLX_DEBUG
	...
	public override function update(elapsed:Float):Void
	{
		...
		if (visible)
		{
			...
			if (status != SpriteButtonStatus.NORMAL && (!overlapFound || (currentInput != null && currentInput.justReleased)))
			{
		...
		for (camera in cameras)
		{
			for (touch in touches)
			{
				...
				function updateTouchID():Void
				{
					...
					{
						final prevButton:Null<SpriteButton> = buttonsTouchID.get(touchID);
	...
	{
		return !(status != SpriteButtonStatus.NORMAL && (!check || (currentInput != null && currentInput.justReleased)));
	}
	...
	function updateStatus(newInput:IFlxInput):Void
	{
		...
		else if (status == SpriteButtonStatus.NORMAL && !ignoreDownHandler)
		{
			if (newInput.pressed)
			{
				onDownHandler();
	...
	{
		status = SpriteButtonStatus.PRESSED;
